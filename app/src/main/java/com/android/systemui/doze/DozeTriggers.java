...
public class DozeTriggers implements DozeMachine.Part {
    ...
    private void onProximityFar(boolean far) {
        ...
        final State state = mMachine.getState();
        final boolean paused = (state == State.DOZE_AOD_PAUSED);
        final boolean pausing = (state == State.DOZE_AOD_PAUSING);
        final boolean aod = (state == State.DOZE_AOD);

        if (state == State.DOZE_PULSING
        ...
        if (far && (paused || pausing)) {
            ...
            mMachine.requestState(State.DOZE_AOD_PAUSING);
        }
    }
     ...
     *              transitions.
     */
    private void onWakeScreen(boolean wake, @Nullable State state, int reason) {
        ...
        if (wake) {
            ...
            boolean paused = (state == State.DOZE_AOD_PAUSED);
            boolean pausing = (state == State.DOZE_AOD_PAUSING);
    ...
    @Override
    public void transitionTo(State oldState, State newState) {
        switch (newState) {
            case INITIALIZED:
            ...
            case DOZE_AOD:
                mAodInterruptRunnable = null;
                mWantProxSensor = newState != State.DOZE;
                mWantSensors = true;
                mWantTouchScreenSensors = true;
                if (newState == State.DOZE_AOD && !sWakeDisplaySensorState) {
    ...
    private void checkTriggersAtInit() {
        if (mUiModeManager.getCurrentModeType() == Configuration.UI_MODE_TYPE_CAR
            ...
            mMachine.requestState(State.FINISH);
        }
    }
    ...
    private void requestPulse(final int reason, boolean performedProxCheck,
        ...
        if (dozeState == State.DOZE_PULSING
            ...
            mMachine.requestState(State.DOZE_PULSING_BRIGHT);
            return;
        }
    ...
    private boolean canPulse() {
        return mMachine.getState() == State.DOZE
                || mMachine.getState() == State.DOZE_AOD
                || mMachine.getState() == State.DOZE_AOD_DOCKED;
    }
    ...
    private DozeHost.Callback mHostCallback = new DozeHost.Callback() {
        ...
        public void onPowerSaveChanged(boolean active) {
            if (mDozeHost.isPowerSaveActive()) {
                mMachine.requestState(State.DOZE);
            } else if (mMachine.getState() == State.DOZE
